...
using namespace wp81DebugLogger;
...
void wp81DebugLogger::MainPage::AppBarButton_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
...
void wp81DebugLogger::MainPage::Install()
{
	TextTest->Text += L"Create driver WP81DebugLogger in registry... ";
	...
	HKEY wp81driverKey = {};
	retCode = win32Api.RegCreateKeyExW(servicesKey, L"wp81debuglogger", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &wp81driverKey, NULL);
	if (retCode != ERROR_SUCCESS)
	{
		debug(L"Error RegCreateKeyExW 'wp81debuglogger': %d\n", retCode);
		TextTest->Text += L"Failed\n";
		return;
	}
	...
	wcscpy_s((WCHAR*)ValueData, 128, L"WP81 DebugLogger driver");
	...
	ZeroMemory(ValueData, 256);
	wcscpy_s((WCHAR*)ValueData, 128, L"Wp81DebugLogger");
	...
	*(PDWORD)ValueData = 1; // SERVICE_KERNEL_DRIVER (started by the IoInitSystem)
	...
	*(PDWORD)ValueData = 1; // 	A kernel-mode device driver
	...
	if (retCode != ERROR_SUCCESS)
	{
		debug(L"Error RegCloseKey 'wp81debuglogger': %d\n", retCode);
		TextTest->Text += L"Failed\n";
		return;
	}
	...
	Uri^ uri = ref new Uri("ms-appx:///Payload/wp81debuglogger.sys");
	create_task(StorageFile::GetFileFromApplicationUriAsync(uri)).then([=](task<StorageFile^> t)
	{
		...
		debug(L"FilePath : %ls\n", filePath->Data());
		if (!win32Api.CopyFileW(filePath->Data(), L"C:\\windows\\system32\\drivers\\wp81debuglogger.sys", FALSE))
		{
